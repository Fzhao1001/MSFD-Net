...
model_dir = "./trained_models/mfediff/LoLv1|"
models_={
    ...
    for step in range(start_step+1,opt.steps+1):
        ...
        if not opt.no_lr_sche:
            ...
            for param_group in optim.param_groups:
                param_group["lr"] = lr
        x,y=next(iter(loader_train))#读数据
        ...
        l_per=criterion[1](x_out[0],y)
        l_si = criterion[2](x_out[0], y)
        l_cons=criterion[3](x_out,y, x)

        loss=l_rec+l_per*0.3+1.5*l_cons
        ...
        if step % opt.eval_step ==0 :
            ...
            if ssim_eval > 0.84 and psnr_eval >23  :
                max_ssim=max(max_ssim,ssim_eval)
                max_psnr=max(max_psnr,psnr_eval)
                torch.save({
                            'model':net.state_dict()
                },model_dir+'p'+str(round(psnr_eval,3))+'s'+str(round(ssim_eval,3))+'m'+".pt")
